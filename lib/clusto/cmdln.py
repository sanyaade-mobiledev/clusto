...
class CmdlnConfigParserError(Exception):
...
class Cmdln(RawCmdln):
    ...
    ## used to decide if we should try to process a CmdlnConfigFile as well
    withconfig = False

    def get_optparser(self):
        parser = RawCmdln.get_optparser(self)
    ...
    def postoptparse(self):
        """
        adds support for config file options.
        """

        RawCmdln.postoptparse(self)

        if self.withconfig:
            if self.options.configfile:
                self.config = CmdlnConfigParser(self.name,
                                                filename=self.options.configfile)
            else:
                self.config = CmdlnConfigParser(self.name)

            if self.config.has_section(self.name):
                for opt, val in self.config.items(self.name):
                    if hasattr(self.options, opt):
                        continue
                    ## some magic so we can set the appropriate type
                    ## specified by the option parser
                    parseropt = self.optparser.get_option('--' + opt)
                    if parseropt:
                        try:
                            val = parseropt.check_value('--' + opt, val)
                        except:
                            raise CmdlnConfigParserError('option %s in section'
                                                         ' %s of the config'
                                                         'file is of the '
                                                         'wrong type.  '
                                                         'Should be a %s'
    ...
    def _dispatch_cmd(self, handler, argv):
        ...
        if co_argcount == 2:   # handler ::= do_foo(self, argv)
            ...
            if self.withconfig:
                ## this is a lame way to get the cmdname from the handler
                try:
                    cmdname = re.match(r'^_?do_(.*)$',
                                       handler.im_func.func_name).groups()[0]
                except:
                    cmdname = None

                if self.config.has_section(cmdname):
                    for opt, val in self.config.items(cmdname):
                        if hasattr(self.options, opt):
                            continue
                        ## some magic so we can set the appropriate type
                        ## specified by the option parser
                        parseropt = optparser.get_option('--' + opt)
                        if parseropt:
                            try:
                                val = parseropt.check_value('--' + opt, val)
                            except:
                                raise CmdlnConfigParserError(
                                    'Option %s in section %s of the config'
                                    'file is of the wrong type. It Should be '
                                    'a %s.'
                                    % (opt, cmdname, parseropt.type))

                        optparser.set_default(opt, val)
...
class CmdlnConfigParser(ConfigParser.SafeConfigParser):
    """
    Opens a config file suitable for cmdln.

    Config files will be standard python ini format with one section sharing
    the name of the program for global options, and the rest of the sections
    having the name of a subcommand for subcommand options.
    ...
    Then the configuration parameters can be merged into the optionparser as
    default values.
    """

    def __init__(self, progname=None, filename=None, *args, **kwargs):

        if not progname:
            raise CmdlnConfigParserError('No progname specified.')
        
        ConfigParser.SafeConfigParser.__init__(self, *args, **kwargs)
        
        # we may want to iterate over several possible locations but I'll
        # keep it simple just for now.

        if filename:
            filenames = (filename,)
        else:
            filenames = ("/etc/%s/%s.ini" % (progname, progname),)
